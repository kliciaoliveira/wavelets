/* PROGRAMA ONDALETAS BIDIMENSIONAIS PARA O MODELO M1
DESCRIÇÃO: O modelo foi reescrito aplicando a integral numérica ao modelo e lendo o arquivo de 
...
double integral( double inicialx, double finalx, double inicialy, double finaly, vector<vector<double> > matrix )
{
	...
	for (double x = inicialx; x < finalx; x+=partex) {
		for (double y =inicialy; y < finaly; y+=partey) {
			...
			/* O acesso à matriz de valores do modelo é feito por números inteiros.
			Temos que transformar os valores de x e y em valores inteiros antes de acessar a matriz */
			int x_matriz = floor(x);
